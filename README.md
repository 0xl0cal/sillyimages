...
Совпадения ищутся в промпте генерации картинки. Имя должно встречаться целиком; регистр букв не важен. Все дополнительные ключи должны присутствовать в этом же промпте. Включённая запись может содержать картинку, описание или оба поля. Переключатель **Send reference descriptions from lorebook** добавляет описания в итоговый промпт; отправка картинок зависит от поддержки референсов и лимита выбранной модели.

В окне последнего запроса раздел **Исключённые референсы** показывает причину исключения каждой записи. **Промпт для поиска совпадений** содержит точный текст, проверяемый по именам и условиям референсов.
