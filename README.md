...
- **Модель** — список подтягивается кнопкой 🔄 из каталога выбранного провайдера.
...
- **Лорбуки** — именованные коллекции дополнительных референсов. Кнопки рядом со списком позволяют создавать, переименовывать, импортировать и удалять коллекции.
...
**Простой режим** ищет совпадения во всех лорбуках по именам и алиасам через запятую. Имя должно встречаться целиком в промпте картинки; регистр букв не важен. У каждой карточки есть галочка включения и режим отправки. **Расширенный режим** учитывает переключатели лорбуков, regex, дополнительные ключи и приоритет. Все дополнительные ключи должны присутствовать в промпте картинки.

Включённая запись может содержать картинку, описание или оба поля. Переключатель **Send reference descriptions from lorebook** добавляет описания в итоговый промпт. Отправка картинок зависит от поддержки референсов и лимита выбранной модели.

Проверка совпадений и запросов: `node --experimental-vm-modules --test tests/reference-matching.test.cjs`. Тесты используют настройки в памяти и имитацию ответов HTTP.

Простой режим использует порядок записей в библиотеке, расширенный — убывающий приоритет. Картинки отбираются в пределах лимита референсов провайдера.
